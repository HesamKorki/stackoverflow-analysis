...
eu |>
  ggplot() +
  geom_histogram(aes(CompTotal), bins=120) +
  ...
  annotate(
    "label",
    x = 10, y = 1600,
    label = paste("medain:", scales::comma(mean(eu$CompTotal), suffix = " \u20AC")),
...
eu |> 
  ggplot() +
  aes(x=fct_lump(Country, n=8), y=CompTotal) +
  geom_boxplot() + 
  scale_y_log10() + 
  labs(x=NULL, y=NULL, title="Gross Salary Distribution by Country") +
  adjust_spacing
```
